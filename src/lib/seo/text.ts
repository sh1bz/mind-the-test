...
export const llmsFull = () => {
	...
	out.push(`## How ${SITE.name} works\n\n${HOW_IT_WORKS.map((s) => `${s.n}. ${s.t}. ${s.d}`).join('\n')}\n\nPricing: free for ${SITE.free.questions} questions and ${SITE.free.mocks} mock exam; ${SITE.price} once unlocks all ${SITE.questions} questions and unlimited mocks. ${SITE.url}/pricing/\n`);
	out.push(`## FAQ\n\n${FAQ.map((f) => `### ${f.q}\n\n${f.a}`).join('\n\n')}\n`);
